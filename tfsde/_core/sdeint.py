...
def sdeint(sde,
    ...
    sde, y0, ts, bm, method, options = check_contract(sde, y0, ts, bm, method, options, names)
...
def check_contract(sde, y0, ts, bm, method,  options, names):
    ...
    state_sizes = []
    noise_sizes = []
    batch_sizes.append(y0.shape[0])
    state_sizes.append(y0.shape[1])
    if bm is not None:
        if len(bm.shape) != 2:
    ...
    if hasattr(sde, 'f'):
        has_f = True
        f_drift_shape = tuple(sde.f(ts[0], y0).shape)
        _check_2d('Drift', f_drift_shape)
    if hasattr(sde, 'g'):
        has_g = True
        g_diffusion_shape = tuple(sde.g(ts[0], y0).shape)
        _check_2d_or_3d('Diffusion', g_diffusion_shape)
    if hasattr(sde, 'f_and_g'):
        has_f = True
        has_g = True
        _f, _g = sde.f_and_g(ts[0], y0)
        f_drift_shape = tuple(_f.shape)
        g_diffusion_shape = tuple(_g.shape)
        _check_2d('Drift', f_drift_shape)
        _check_2d_or_3d('Diffusion', g_diffusion_shape)


    if not has_f:
        raise ValueError("sde must define at least one of `f`, `f_and_g`, or `f_and_g_prod (support removed)`. (Or possibly more "
                         "depending on the method chosen.)")
    if not has_g:
        raise ValueError("sde must define at least one of `g`, `f_and_g`, `g_prod (support removed)` or `f_and_g_prod (support removed)`. (Or possibly "
