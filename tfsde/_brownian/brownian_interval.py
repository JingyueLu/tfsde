...
def _randn(size, dtype, seed):
    ...
    return generator.normal(shape=size, dtype=dtype)


def _is_scalar(x):
    return isinstance(x, int) or isinstance(x, float) or (isinstance(x, tf.Tensor) and np.prod(x.shape.as_list()) == 1)
...
def _check_tensor_info(*tensors, size, dtype):
    ...
    tensors = list(filter(tf.is_tensor, tensors))
...
def _davie_foster_approximation(W, H, h, levy_area_approximation, get_noise):
    if levy_area_approximation in (LEVY_AREA_APPROXIMATIONS.none, LEVY_AREA_APPROXIMATIONS.space_time):
        ...
        A = tf.expand_dims(H, axis=-1) * tf.expand_dims(W, axis=-2) - tf.expand_dims(W, axis=-1) * tf.expand_dims(H, axis=-2)
        noise = get_noise()
        noise = noise - tf.linalg.matrix_transpose(noise)  # noise is skew symmetric of variance 2
...
def _H_to_U(W: tf.Tensor, H: tf.Tensor, h: float) -> tf.Tensor:
    return h * (.5 * W + H)
...
class _Interval:
    ...
    def _randn(self, seed):
        ...
        return _randn(size, self._top._dtype, seed)
    ...
    def _randn_levy(self):
        ...
        return _randn(size, self._top._dtype, self._a_seed())
...
class BrownianInterval(brownian_base.BaseBrownian, _Interval):
    ...
    def __init__(self,
        ...
        if not _is_scalar(t0):
            raise ValueError('Initial time t0 should be a float or 0-d tf.Tensor.')
        if not _is_scalar(t1):
            raise ValueError('Terminal time t1 should be a float or 0-d tf.Tensor.')
        if dt is not None and not _is_scalar(dt):
            raise ValueError('Expected average time step dt should be a float or 0-d tf.Tensor.')
    ...
    def __call__(self, ta, tb=None, return_U=False, return_A=False):
        ...
        if ta == tb:
            W = tf.zeros(self._size, dtype=self._dtype)
            H = None
            A = None
            if self._have_H:
                #H = torch.zeros(self._size, dtype=self._dtype, device=self._device)
                H = tf.zeros(self._size, dtype=self._dtype)
            if self._have_A:
                ...
                A = tf.zeros(size, dtype=self._dtype)
        else:
            ...
            if len(intervals) > 1:
                ...
                for interval in intervals[1:]:
                    ...
                    if self._have_A and len(self._size) not in (0, 1):
                        ...
                        A = A + Ai + 0.5 * (tf.expand_dims(W, axis=-1) * tf.expand_dims(Wi, axis=-2) - tf.expand_dims(Wi, axis=-1) * tf.expand_dims(W, axis=-2))
                    W = W + Wi
